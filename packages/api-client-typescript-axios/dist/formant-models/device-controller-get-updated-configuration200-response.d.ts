/**
 * Formant admin-api
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.193.2
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
import { DeviceControllerPutConfigurationRequest } from './device-controller-put-configuration-request';
/**
 *
 * @export
 * @interface DeviceControllerGetUpdatedConfiguration200Response
 */
export interface DeviceControllerGetUpdatedConfiguration200Response {
    /**
     *
     * @type {DeviceControllerPutConfigurationRequest}
     * @memberof DeviceControllerGetUpdatedConfiguration200Response
     */
    'desiredConfiguration'?: DeviceControllerPutConfigurationRequest | null;
}
